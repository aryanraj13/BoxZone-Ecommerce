        ...
        <div ref={ref} className='w-72 h-full sideCart absolute top-0 right-0 bg-purple-300 px-8 py-10 transition-transform translate-x-full transform'>
          ...
          <ol className='list-decimal font-semibold'>
            ...
            {Object.keys(cart).map((k)=>{return<li key={k}>
              ...
              <div className='font-semibold flex items-center justify-center w-1/3 text-lg'><AiFillMinusCircle onClick={()=>{removeFromCart(k,1,cart[k].price,cart[k].name,cart[k].size,cart[k].variant)}} className='cursor-pointer text-purple-600' /><span className='mx-2 text-sm'>{cart[k].qty}</span><AiFillPlusCircle onClick={()=>{addToCart(k,1,cart[k].price,cart[k].name,cart[k].size,cart[k].variant)}} className='cursor-pointer text-purple-600'/></div>
              </div>
            </li>})}
...
hover:bg-purple-950 rounded text-sm"><BsFillBagCheckFill className='m-1'/>Checkout</button>
          <button onClick={clearCart} className="flex mx-2 text-white bg-purple-600 border-0 py-2 px-2 focus: outline-no
