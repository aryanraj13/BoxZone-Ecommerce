  ...
  let sticks ={}
    for(let item of products){
        if(item.title in sticks){
            if(!sticks[item.title].color.includes(item.color) && item.availableQty>0){
                sticks[item.title].color.push(item.color)
            }

            if(!sticks[item.title].color.includes(item.size) && item.availableQty>0){
